...
{
    std::string stmp = str;
    transform(stmp.begin(), stmp.end(), stmp.begin(), (int (*)(int))toupper);
    return stmp;
}
...
{
    std::string stmp = str;
    transform(stmp.begin(), stmp.end(), stmp.begin(), (int (*)(int))tolower);
    return stmp;
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned acolor = 0xFF00FF;
    for (int i = 1; i < argc; ++i) {
        std::string stmp = ToUpper(argv[i]);
        if (i == argc - 1) {
            std::cout << "Bad Command!" << std::endl;
            return 1;
        }
        ...
        if (stmp == "-C") {
            ...
            is >> std::hex >> acolor >> std::dec;
            continue;
        }
    }
    if (outfile == "") outfile = infile + ".converted.bmp";
    ...
    int pnglen = image.size() / 4;
    for (int i = 1; i <= pnglen; i++) {
        if (image[i*4-1] == 0) {
                image[i*4-2] = acolor & 0x0000FF;
                image[i*4-3] = (acolor & 0x00FF00) >> 8;
                image[i*4-4] = (acolor & 0xFF0000) >> 16;
        }
        //printf("%2x %2x %2x %d\n", image[i*4-4], image[i*4-3], image[i*4-2], image[i*4-1]);
    }
